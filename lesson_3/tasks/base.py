...
class Base:
    ...
    async def get_token_price(self, token1='ETH',token2='USDT', retries: int = 1):
        token1 = token1.upper()
        token2 = token2.upper()
        for _ in range(retries):
            try:
                async with aiohttp.ClientSession() as session:
                    logger.info(
                        f'{self.client.account.address} | getting {token1} price')
                    async with session.get(f'https://api.binance.com/api/v3/depth?limit=1&symbol={token1}{token2}') as r:
                        if r.status != 200:
                ...
                logger.error(
                    f'{self.client.account.address} | getting {token1} price: {e}')
